...
**Ограничение (унаследовано от бывшего §8):** блок-ожидание внутри `if`-ветки поддерживается (переход просто идёт в состояние после `if`, независимо от того, какая ветка выполнилась). Блок-ожидание внутри `controls_repeat_ext`/`controls_whileUntil`/`controls_for` **не поддерживается** — такой блок отключается (`setDisabledReason(true, 'wait_in_loop')`) с предупреждением «В Lua ожидание внутри цикла не поддерживается».

...
## 2.2 Единицы цвета в Python `led_control` — [ИСПРАВЛЕНО И ОТМЕНЕНО, см. ниже] (2026-09-13, отменено 2026-09-14)

**Отменено.** Первоначально по примерам с GitFlic (`pioneer-python-example`) показалось, что `led_control(r,g,b)` принимает 0..1 (там встречаются вызовы вида `r=0.1`). На основании этого симулятор и Python-генераторы цвета были переведены на 0..1 (коммит `cd85c06`).

Это оказалось неверно. Проверка настоящего pip-пакета `pioneer-sdk` (установлен локально, `pip show pioneer-sdk` → версия 0.5.3, official `github.com/geoscan/pioneer_sdk`) показала явную валидацию в исходнике (`piosdk.py`, метод `led_control`):
```python
if r < 0 or r > 255 or g < 0 or g > 255 or b < 0 or b > 255:
    raise ValueError(f"Arguments 'r', 'g', 'b' must have value in [0, 255]...")
```
То есть реальный SDK ожидает **0..255**, как и было в симуляторе изначально; примеры на GitFlic с `r=0.1` — это баг/неточность в примерах Geoscan (значение технически проходит проверку диапазона, просто светит почти невидимо тускло), а не отражение реального контракта API. Исходный код SDK — источник истины более высокого приоритета, чем стороннее example-репо.

**Откат сделан 2026-09-14**: `pioneer_led_control` в `pioneer-js-bridge.ts` снова не масштабирует r/g/b; все Python-генераторы цвета (`led_all`/`led_index` в `pioneer-sdk2-definitions.ts`, `pioneer_led_all`/`pioneer_led_index` в `pioneer/blocks/leds.ts`) снова передают 0..255 без деления; `python-api-docs.ts` восстановлен. Учебный контент в `public/modules/ui/mission-guide/**` **не трогали вообще** (ни в исходном "исправлении", ни в откате) — он всё это время уже был написан правильно (`r=255`), это подтверждено повторной проверкой. Единственное реальное (не связанное с этим эпизодом) расхождение в 0..1 — старый `teaching-python-definitions.ts` (`pioneer.led_control(255, *[c / 255 for c in color][::-1])`), это баг, который просто ещё не почищен вместе с остальным legacy-кодом в Фазе 9.

**Важно:** если где-то (в этой или соседней сессии) была запущена задача "привести LED-единицы в mission-guide к 0..1" — это нужно остановить/отменить, она бы вносила ошибку в уже корректный контент.
...
2. Берёт **только** цепочку под `pioneer_start` (если блоков `pioneer_start` несколько, используется первый, остальные получают предупреждение) и все `pioneer_on_event`. Прочие верхнеуровневые блоки: определения функций (`procedures_def*`) генерируются как обычно, остальные блоки-сироты игнорируются и получают `setWarningText('Блок не подключён к «Начало программы»')`.
   ...
   **[найдено и исправлено 2026-09-14]** Реальный баг: предупреждение выставлялось только блокам из `otherBlocks` (текущим "сиротам"), но никогда не снималось явно с блоков, которые раньше были сиротами и потом подключились — цикл просто больше не проходил мимо них. Пример: свободный `math_number` на холсте получает предупреждение; пользователь подключает его к входу `X` блока `pioneer_go_to` (который уже в цепочке от `pioneer_start`) — блок больше не top-level, но предупреждение с него никто не снял, и оно оставалось висеть на уже правильно подключённом блоке. Исправлено: перед перерасчётом `workspace.getAllBlocks(false).forEach(b => b.setWarningText(null))` снимает это предупреждение со всех блоков (это единственное место в pioneer_*, где `setWarningText` вообще используется, так что снимать его у всех безопасно), и уже после этого предупреждение навешивается заново только на реальных сирот.
...
**[пересмотрено 2026-09-14]** `local __t0 = time()` и блок `__loop_guard` в прологе выше — тоже условные, тем же принципом, что LED/position-хелперы и Python-хелперы ожидания (§4.4): пустая программа без циклов и без `pioneer_time` не должна тащить за собой ни то, ни другое. `buildLuaProgram` (`targets/lua-runtime.ts`) проверяет по тексту (`headerDefinitions`/`segmentsCode`/`eventBranches`/`transitionBranches`), встречается ли там `__t0` или `__loop_guard(`, и печатает объявление, только если да — это то же самое расхождение с исходным эталоном, которое было в Python-прологе (владелец сравнил сгенерированный код с официальными примерами Geoscan и справедливо назвал безусловный вариант «кошмаром для новичка»).

**[пересмотрено 2026-09-14, два режима вместо одного]** Владелец сравнил FSM-вывод с реальными примерами Geoscan/TRIK и справедливо заметил: для самого частого случая — простой линейной цепочки, где ни одно имя события не повторяется — вся машинерия лишняя. В рукописных скриптах там нет ни `action`, ни `__state`, ни `__advance()`: пришедшее событие САМО однозначно указывает, какой шаг выполнять дальше, потому что встречается в программе ровно один раз, и запоминать «на каком шаге мы сейчас» не нужно. Поэтому Lua-таргет теперь компилирует **в два режима**, с автоматическим выбором:

- **Плоский** (`buildFlatLuaProgram`, `targets/lua-runtime.ts`): код первого шага печатается прямо на верхнем уровне, а каждое ожидание события становится независимой СОСЕДНЕЙ (не вложенной!) ветвью `if event == Ev.X then <продолжение> end` внутри `callback(event)`. Вложенность появляется только у `pioneer_wait`: `Timer.callLater` обязан обернуть продолжение, потому что второй таймер физически нельзя завести до того, как выполнится тело первого. `function callback(event)` печатается всегда, даже пустым, — по его наличию `lua/mission-guard.ts` решает, пропускать ли больше одной команды миссии.
- **FSM** (`buildLuaProgram`, эталон выше) — без изменений, байт-в-байт как раньше.

**Правило выбора (`isFlatEligible`, `targets/lua-fsm.ts`): плоский режим применяется тогда и только тогда, когда имена ожидаемых событий во всей цепочке не повторяются.** Повтор — это настоящая неоднозначность: две `pioneer_go_to` подряд обе ждут `Ev.POINT_REACHED`, и плоская ветка `if event == Ev.POINT_REACHED then <вторая точка> end` сработала бы уже на достижении ПЕРВОЙ точки, потому что без `__state` «первый раз» от «второго» не отличить. Такие программы уходят в полный FSM. Переходы по времени на выбор режима не влияют вообще: сколько бы `pioneer_wait` ни было, каждый из них — своё отдельное замыкание `Timer.callLater`, неоднозначности они не создают. Ноль переходов (например, программа только из LED-блоков) — тривиально плоский случай: остаётся тело программы и пустой `callback`.

Пример плоского вывода для цепочки «моторы → взлёт → точка (1, 0, 1) → ждать 2с → посадка» (эталон владельца, зафиксирован байт-в-байт в `tests/pioneer-blockly-lua-fsm.test.ts`):
...
Последняя ветка (`COPTER_LANDED`) с пустым телом — это терминальный шаг: после последнего блока-ожидания в программе ничего нет. В FSM-режиме ему соответствует такое же пустое последнее состояние (и пустой `_FINAL_NODE` в референсе TRIK из §2.1). Отступ тела `callback` — четыре пробела, как в рукописных примерах, а не `generator.INDENT` (у Blockly это два пробела); FSM-режим намеренно оставлен на `generator.INDENT`, чтобы его вывод не изменился.

Ограничения §4.3 (ожидание внутри циклов/`if`/`pioneer_on_event`) от режима не зависят: оба работают на одной и той же верхнеуровневой линейной цепочке, а ветки `pioneer_on_event` в обоих режимах остаются независимыми побочными обработчиками и печатаются в `callback` первыми. Условный пролог (`__t0`, `__loop_guard`) работает в плоском режиме тем же текстовым поиском маркера.
...
**Вне скоупа v1**, но зафиксировать в бэклоге в конце этого файла при завершении: видеопоток — запись видео и живой просмотр (остаются вне v1; одиночный снимок добавлен 2026-09-14, см. §11), CV/ИИ-блоки из `pioneer-sdk2-definitions.ts` (вызывают несуществующие функции), GPIO/UART/SPI/mailbox (Lua), `go_to_local_point_body_fixed`, `send_rc_channels`, `lua_script_control`.
...
- Не расширять скоуп (CV, GPIO) без запроса. Камера: одиночный снимок запрошен владельцем и добавлен 2026-09-14 (§11); видео и живой просмотр по-прежнему вне скоупа.
...
## 11. Камера — одиночный снимок (2026-09-14)

Владелец подтвердил реальный Lua camera API с официальной документации Geoscan
(`camera.requestMakeShot()` / `camera.checkRequestShot()` / запись видео) и
запросил блок «Сделать снимок» для обоих таргетов, сузив первую очередь до
одного снимка (видео и живой просмотр — по-прежнему вне скоупа, см. правки
в §5 и §7 выше). К этому моменту Lua-рантайм для камеры уже был полностью
реализован (`modules/lua/hardware/camera.ts`: рендерит FPV-камеру дрона в
offscreen-канвас и реально скачивает PNG через браузер) — просто без блока
Blockly, который бы его вызывал.

**Третий тип перехода в Lua FSM.** `splitLuaChain()` (`targets/lua-fsm.ts`)
до этого различал только `event` (ожидание `Ev.*` от автопилота) и `timer`
(фиксированная задержка `Timer.callLater`). `camera.checkRequestShot()` —
ни то, ни другое: это не событие автопилота (камера с `ap`/`Ev` вообще не
связана) и не фиксированная задержка (готовность наступает через
неизвестное короткое время — следующий `requestAnimationFrame` плюс
кодирование PNG). Добавлен третий вид перехода, `poll` (маркер
`__wait_poll(condExpr)`): замыкание `__poll_sN` перезаводит само себя через
`Timer.callLater(0.05, ...)`, пока `condExpr` ложно, и продвигает `__state`
(FSM-режим) или просто исполняет продолжение (плоский режим), когда оно
истинно. Интервал 0.05 с — тот же порядок, что у `_pioneer_position`
(`blocks/sensors.ts`), а не 0.1 с из `pollWhile()` (`blocks/flight.ts`): тот
более длинный интервал защищает от двух команд автопилота в одном тике
симулятора, а `requestMakeShot()` командой автопилота не является и в
`recordTickCommand`/`failSimultaneousCommands` не попадает — защищаться не
от чего. `isFlatEligible()` не изменился по сути: `poll`-переходы, как и
`timer`, безопасно повторяются (каждый — своё независимое замыкание), в
проверку на дублирующиеся имена событий не попадают.

**Python: новый мост для скачивания файла.** У `pioneer_sdk` в Python нет
метода «сохранить снимок» — только непрерывный `Camera.get_frame()`
(официальный пример: `docs/imported/Python_files/frames_from_camera.py`).
Файловая система Pyodide живёт в памяти вкладки и ученику недоступна, а
`cv2.imwrite` в симуляторе — заглушка (`pioneer-sdk-cv-prelude.ts`), так что
у Python-таргета до этой задачи вообще не было способа отдать кадр ученику
за пределы скрипта. Добавлена `downloadDroneCameraPhoto()`
(`pioneer-js-bridge-camera.ts`) — copy-paste `triggerBrowserDownload()` из
Lua-модуля камеры (тот же `createObjectURL` + `<a download>` +
`revokeObjectURL`; общий хелпер не заводили, чтобы не тащить Lua/fengari-
рантайм в Python-мост ради шести строк), вызывается из блока через
`js.pioneer_camera_save_photo(camera._id)`. Блок Python-таргета генерирует
`camera.get_frame()` первым шагом (не только ради байтов — если камера ни к
чему не подключена, `get_frame()` вернёт пустые байты и ученик увидит
ошибку в своём коде, а не молча пустой файл) и только потом сохраняет то же
самое изображение через мост (оба пути используют один 100-мс кеш кадра в
`pioneer-js-bridge-camera-render.ts` — скачивается буквально тот кадр,
который получила программа).

**Что не проверить автоматизированным браузером:** реальное появление файла
в папке загрузок пользователя — это решение браузера, а не что-то,
перехватываемое из `read_console_messages`/`read_network_requests`. Проверено
через лог-сообщения (`Camera: Снимок сохранен как ...` / `Camera: Не удалось
сохранить снимок: ...`) и отсутствие рантайм-ошибок при живом прогоне обоих
таргетов.

...
import net from 'net';
import { getExternalPythonBridgeState, recordExternalPythonBridgeEvent } from './external-python-bridge.js';
...
function encodeMavlinkV2Message(sequence, systemId, componentId, messageId, payload) {
...
function parseSingleMavlinkFrame(datagram) {
...
function parseCommandLong(payload) {
...
function parseSetPositionTargetLocalNed(payload) {
...
function parseRcChannelsOverride(payload) {
...
function mapAutopilotStateToCustomMode(state) {
    switch (state) {
        case 'ARMED':
            return 11;
        case 'TAKEOFF':
...
function mapAutopilotStateToSystemStatus(state) {
    switch (state) {
        case 'TAKEOFF':
        case 'MISSION':
        case 'LANDING':
...
function isArmedAutopilotState(state) {
    return state === 'ARMED' || state === 'TAKEOFF' || state === 'MISSION' || state === 'LANDING';
}
function emitBridgeEvent(connection, sessionId, method, args = [], kwargs = {}) {
...
function emitCameraBridgeEvent(connection, sessionId, method) {
    recordExternalPythonBridgeEvent({
        sessionId,
        droneName: connection.droneName,
        droneIp: connection.droneIp,
...
class MavlinkUdpBridge {
    ...
    handleDatagram(datagram, remoteAddress, remotePort) {
    ...
    touchSession(remoteAddress, remotePort, systemId, componentId) {
    ...
    sendMessage(messageId, payload) {
        ...
        const packet = encodeMavlinkV2Message(this.session.outgoingSequence++ & 0xFF, MAVLINK_SYS_ID, MAVLINK_COMP_ID, messageId, payload);
    ...
    sendCommandAck(command, result = 0) {
    ...
    handleCommandLong(payload) {
        ...
        switch (command.command) {
            case MAV_CMD_COMPONENT_ARM_DISARM:
                emitBridgeEvent(this.connection, this.session.sessionId, command.param1 >= 0.5 ? 'arm' : 'disarm');
    ...
    handleSetPositionTargetLocalNed(payload) {
        const command = parseSetPositionTargetLocalNed(payload);
        if (!command) {
            return;
        }
        if (command.typeMask === GO_TO_LOCAL_POINT_MASK) {
            emitBridgeEvent(this.connection, this.session.sessionId, command.coordinateFrame === MAV_FRAME_BODY_FRD ? 'go_to_local_point_body_fixed' : 'go_to_local_point', [], {
                x: command.y,
                y: command.x,
                z: -command.z,
                yaw: command.yaw
            ...
            emitBridgeEvent(this.connection, this.session.sessionId, command.coordinateFrame === MAV_FRAME_BODY_FRD ? 'set_manual_speed_body_fixed' : 'set_manual_speed', [], {
                vx: command.vy,
                vy: command.vx,
                vz: -command.vz,
    ...
    handleRcChannelsOverride(payload) {
...
class CameraTcpBridge {
    ...
    handleSocket(socket) {
        ...
        void import('node:fs').then((fs) => { let u = 'http://127.0.0.1:7778/event', s = 'camera-udp-timeout'; try {
            const e = fs.readFileSync('.dbg/camera-udp-timeout.env', 'utf8');
    ...
    stopClient(client, destroySocket) {
...
function ensureBridgeConnections(connections) {
...
export function registerMavlinkBridgeRoutes(app) {
    ensureBridgeConnections(DEFAULT_CONNECTIONS);
    app.post('/api/mavlink-bridge/connections', (req, res) => {
        const connections = Array.isArray(req.body?.connections) ? req.body.connections : [];
        ensureBridgeConnections(connections.map((entry) => sanitizeRegistration(entry)));
        return res.json({
            ok: true,
...
export function stopAllMavlinkBridges() {
